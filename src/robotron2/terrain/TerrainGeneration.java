		...
		if(HvlMenu.top() == MenuManager.game) {
			for(int i = 0; i < blocks.size(); i++) {
				...
				if(blocks.get(i).getCollidable()) {
					for (Bullet bullet : BulletLogic.bulletTotal) {
						if(bullet.isFired()) {
							if ((bullet.getxPos() < blocks.get(i).getxPos() + (Block.BLOCK_SIZE/2)) 
									&& (bullet.getxPos() > blocks.get(i).getxPos()-(Block.BLOCK_SIZE/2))
									&& (bullet.getyPos() < blocks.get(i).getyPos()+(Block.BLOCK_SIZE/2))
									&& (bullet.getyPos() > blocks.get(i).getyPos()-(Block.BLOCK_SIZE/2))) {
